    ...
    if is_metadata(download):
        ...
        if task := await get_task_by_gid(gid):
            ...
            if task.listener.select:
                metamsg = "<blockquote>╭ℹ️ <b>Metadata</b>\n╰Downloading Metadata, wait then you can select files. Use torrent file to avoid this wait.</blockquote>"
    ...
    if download.get("followedBy", []):
        ...
        if task := await get_task_by_gid(new_gid):
            ...
            if Config.BASE_URL and task.listener.select:
                ...
                msg = "<blockquote>╭⏸️ <b>Selection</b>\n╰Your download paused. Choose files then press Done Selecting button to start downloading.</blockquote>"
