...
from bot.helper.ext_utils.links_utils import is_telegram_link, is_url, is_magnet, is_gdrive_link, is_rclone_path
...
class TaskRecovery:
    ...
    async def recover_incomplete_tasks(self):
        ...
        if not Config.DATABASE_URL or not Config.AUTO_RESTART_TASKS or self.recovery_in_progress:
            return
        ...
        try:
            ...
            for task_data in incomplete_tasks:
                try:
                    ...
                    LOGGER.error(f"Failed to recover task {task_data.get('link', 'unknown')}: {e}")
                    failed_count += 1
                    
            LOGGER.info(f"Task recovery completed. Recovered: {recovered_count}, Failed: {failed_count}")
            
            # Send summary to owner if there were recoveries
            if recovered_count > 0 or failed_count > 0:
                summary_msg = f"🔄 <b>Task Recovery Summary</b>\n\n"
    ...
    async def _restart_task(self, task_data):
        ...
        try:
            ...
            LOGGER.error(f"Failed to restart task {task_data.get('link', 'unknown')}: {e}")
            return False
