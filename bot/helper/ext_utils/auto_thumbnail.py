...
class AutoThumbnailHelper:
    ...
    async def _get_tmdb_thumbnail(
        ...
        try:
            ...
            thumb_format = getattr(Config, "AUTO_THUMBNAIL_FORMAT", "poster").lower()

            # Try to get backdrop if requested
            if thumb_format == "backdrop" and result.get("backdrop_path"):
                return TMDBHelper.get_backdrop_url(result["backdrop_path"], "w780")

            # Fallback to poster if backdrop not found or if poster was requested
            if result.get("poster_path"):
                return TMDBHelper.get_poster_url(result["poster_path"], "w500")

            # Final attempt at backdrop if poster is somehow missing but backdrop exists
            if thumb_format == "poster" and result.get("backdrop_path"):
                return TMDBHelper.get_backdrop_url(result["backdrop_path"], "w780")
