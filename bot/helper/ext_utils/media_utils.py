...
async def get_md_leech_cmd(file_path):
    ...
    if has_hindi:
        ...
        LOGGER.warning(
            f"MDLeech: No Hindi audio found in {file_path}. Processing without audio."
        )
        # No audio mapped
...
async def take_ss(video_file, ss_nb) -> bool:
    ...
    if duration != 0:
        ...
        try:
            ...
            LOGGER.error(
                f"Error while creating screenshots from video. Path: {video_file}. Error: Timeout some issues with ffmpeg!"
            )
            await rmtree(dirpath, ignore_errors=True)
            return False
        return dirpath
...
async def get_audio_thumbnail(audio_file):
    ...
    try:
        ...
        LOGGER.error(
            f"Error while extracting thumbnail from audio. Name: {audio_file}. Error: Timeout some issues with ffmpeg!"
        )
        return None
    return output
...
async def get_video_thumbnail(video_file, duration):
    ...
    try:
        ...
        LOGGER.error(
            f"Error while extracting thumbnail from video. Name: {video_file}. Error: Timeout some issues with ffmpeg!"
        )
        return None
    return output
...
async def get_multiple_frames_thumbnail(video_file, layout, keep_screenshots):
    ...
    try:
        ...
        LOGGER.error(
            f"Error while combining thumbnails from video. Name: {video_file}. Error: Timeout some issues with ffmpeg!"
        )
        return None
    finally:
