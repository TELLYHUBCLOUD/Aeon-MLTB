...
async def task_init_helper(message, client):
    """
    Consolidated helper for task initialization (Mirror, Clone, Encode, Merge).
    Performs user/permissions check, parses arguments, and handles bulk link extraction.

    Args:
        message: The message object triggering the task.
        client: The Telegram client.

    Returns:
        tuple: (args, input_list, is_bulk, bulk_links) if successful.
        Returns (None, None, None, None) if validation fails (error message is sent automatically).
    """
    ...
    # Parse arguments from input list
    ...
    if not is_bulk:
        ...
        # We pass string 0/0 because extract_bulk_links expects strings in some implementations or handles ints.
        # Based on my read, it converts to int. Let's pass str to be safe as per current code usage.
        bulk_links = await extract_bulk_links(message, str(bulk_start), str(bulk_end))
        if len(bulk_links) > 1:
            is_bulk = True

    return args, input_list, is_bulk, bulk_links
