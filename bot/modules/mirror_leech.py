...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        # AUTO LEECH + AUTO COMPRESS CMD (Preprocessing before init)
        # We need to inject into message.text because task_init_helper reads it
        if self.auto_link and self.message and self.message.text:
            ...
            if not any(x.startswith("http") or "magnet" in x for x in input_list):
                ...
                text[0] = " ".join(input_list)
                self.message.text = "\n".join(text)

        if self.auto_ff and self.message and self.message.text:
            text = self.message.text.split("\n")
            input_list = text[0].split(" ")
