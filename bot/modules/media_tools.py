...
class MediaToolsSelection:
    ...
    async def main_menu(self):
        ...
        buttons.data_button(f"{'✅' if self.sample_video else '❌'} Sample Video", "medtool toggle_sample")
        buttons.data_button(f"{'✅' if self.screen_shots else '❌'} Screenshots", "medtool toggle_ss")
        buttons.data_button(f"{'✅' if self.convert_audio else '❌'} Convert Audio", "medtool toggle_audio")
        buttons.data_button(f"{'✅' if self.convert_video else '❌'} Convert Video", "medtool toggle_video")
        
        # Watermark/Metadata (inherited from global/user settings but toggleable for specific task)
        buttons.data_button(f"{'✅' if self.watermark else '❌'} Watermark", "medtool toggle_wm")
        buttons.data_button(f"{'✅' if self.metadata else '❌'} Metadata", "medtool toggle_md")
    ...
    async def get_selection(self):
        ...
        handler = self.client.add_handler(
            CallbackQueryHandler(pfunc, filters=regex("^medtool") & user(self.user_id)),
            group=-1,
        )
