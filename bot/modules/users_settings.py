...
async def get_user_settings(from_user):
    ...
    buttons.data_button("Metadata key 🗝️", f"userset {user_id} metadata_key")
    buttons.data_button("Watermark text 💧", f"userset {user_id} watermark_key")
    buttons.data_button("Name Substitution 🏷️", f"userset {user_id} name_substitute")
...
async def set_option(_, message, pre_event, option):
    ...
    if option == "excluded_extensions":
        ...
        msg = "Excluded extensions updated ✅"
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        if await aiopath.exists(fpath):
            ...
            await query.answer("⚙️ Old Settings", show_alert=True)
            await update_user_settings(query)
    elif data[2] in [
        "yt_opt",
        "lprefix",
        ...
        thumbpath = f"Thumbnails/{user_id}.jpg"
        buttons = ButtonMaker()
        buttons.data_button("🖼️ Thumbnail", f"userset {user_id} sthumb")
        ...
        if user_dict.get("lprefix", False):
            ...
            lprefix = "None"
        buttons.data_button("✏️ Leech Caption", f"userset {user_id} leech_caption")
        ...
        buttons.data_button("❌ Close", f"userset {user_id} close")
        text = f""">⚙️ Leech Settings

**🔄 Leech Type:** {ltype}
**🖼️ Custom Thumbnail:** {thumbmsg}
...
**✏️ Leech Caption:** `{lcaption}`
"""
        await edit_message(message, text, buttons.build_menu(2), markdown=True)
    elif data[2] == "rclone":
        await query.answer()
        buttons = ButtonMaker()
        buttons.data_button("⚙️ Rclone Config", f"userset {user_id} rcc")
...
**📁 Rclone Path:** `{rccpath}`"""
        ...
        text = f""">⚙️ Gdrive Tools Settings
...
**🔄 Stop Duplicate:** {sd_msg}"""
        ...
        if await aiopath.exists(thumb_path):
            buttons.data_button("🗑️ Delete Thumbnail", f"userset {user_id} thumb")
...
Check all yt-dlp api options from this <a href='https://github.com/yt-dlp/yt-dlp/blob/master/yt_dlp/YoutubeDL.py#L212'>FILE</a> or use this <a href='https://t.me/mltb_official_channel/177'>script</a> to convert cli arguments to api options.
        ...
        buttons.data_button("⬅️ Back", f"userset {user_id} rclone")
        buttons.data_button("❌ Close", f"userset {user_id} close")
        await edit_message(
            message,
        ...
        if await aiopath.exists(token_pickle):
            buttons.data_button(
                "🗑️ Delete token.pickle",
                f"userset {user_id} token_pickle",
            )
        buttons.data_button("⬅️ Back", f"userset {user_id} gdrive")
        buttons.data_button("❌ Close", f"userset {user_id} close")
        await edit_message(
            message,
        ...
        if user_dict.get("index_url", False):
            buttons.data_button("🗑️ Remove Index URL", f"userset {user_id} index_url")
        buttons.data_button("⬅️ Back", f"userset {user_id} gdrive")
        buttons.data_button("❌ Close", f"userset {user_id} close")
        ...
        await edit_message(
            message,
            "Send Leech Filename Prefix ✍️. You can add HTML tags. ⏳ Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
        ...
        if user_dict.get("lcaption", False) or (
            ...
            buttons.data_button("🗑️ Remove Caption", f"userset {user_id} lcaption")
        buttons.data_button("⬅️ Back", f"userset {user_id} leech")
        buttons.data_button("❌ Close", f"userset {user_id} close")
        await edit_message(
            message,
            "Send Leech Filename Caption ✍️. You can add HTML tags. ⏳ Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
        ...
        await edit_message(
            message,
            "🖼️ Send thumbnail layout (widthxheight, 2x2, 3x3, 2x4, 4x4, ...). ⏳ Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
        ...
        await edit_message(
            message,
            "🔑 Send your pyrogram V2 session string for downloading content from private or restricted channels. ⏳ Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
        ...
        await edit_message(
            message,
            "📜 Send excluded extensions separated by space, without a dot at the beginning. ⏳ Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
        ...
        buttons.data_button("⬅️ Back", f"userset {user_id} back")
        buttons.data_button("❌ Close", f"userset {user_id} close")
        emsg = r"""🔠 Word Substitutions. You can add patterns instead of normal text. ⏳ Timeout: 60 sec
...
"""
        ...
        await query.answer()
        buttons = ButtonMaker()
        buttons.data_button("⬅️ Back", f"userset {user_id} upload_paths")
        buttons.data_button("❌ Close", f"userset {user_id} close")
        await edit_message(
            message,
            "📝 Send path name (no spaces) which you will use as a shortcut and the path/id separated by space. You can add multiple names and paths separated by new lines. Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
        ...
        await edit_message(
            message,
            "🗑️ Send path names which you want to delete, separated by space. Timeout: 60 sec",
            buttons.build_menu(1),
            markdown=True,
        )
